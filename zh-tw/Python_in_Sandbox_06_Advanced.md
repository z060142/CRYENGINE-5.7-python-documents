...
Sandbox 原始碼包含一個獨立的 Python 啟動器，用於在 Sandbox 外部執行 Python 腳本。
...
- 可在不啟動完整 Sandbox 的情況下執行 Python 腳本
...
> **限制：** 此啟動器不載入 `sandbox` 模組，因此無法使用 `sandbox.*` 命令。它只提供標準 Python 3.7 環境，適合執行獨立的資料處理腳本。
