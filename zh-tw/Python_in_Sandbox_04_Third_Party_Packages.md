...
| sys.path | 自動包含 site-packages | 需手動加入 |

### 1.2 核心限制

1. **Py_SetPythonHome** 設定為 `Editor/Python/Windows/`，這決定了 Python 尋找 `site-packages` 的位置
2. **無 pip 內建** — 嵌入式 Python 不包含 pip
3. **無 venv** — 因為 Py_SetPythonHome 會覆蓋標準的 site 尋找邏輯
...
Sandbox 已內建 PySide2 (用於 `_CryQt`)。安裝獨立的 PySide2 可能造成衝突。
