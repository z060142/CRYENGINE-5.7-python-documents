...
Sandbox 源代码包含一个独立的 Python 启动器，用于在 Sandbox 外部执行 Python 脚本。
...
- 可在不启动完整 Sandbox 的情况下执行 Python 脚本
...
> **限制：** 此启动器不加载 `sandbox` 模块，因此无法使用 `sandbox.*` 命令。它只提供标准 Python 3.7 环境，适合执行独立的资料处理脚本。
