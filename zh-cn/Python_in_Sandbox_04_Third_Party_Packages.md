...
| sys.path | 自动包含 site-packages | 需手动加入 |

### 1.2 核心限制

1. **Py_SetPythonHome** 设置为 `Editor/Python/Windows/`，这决定了 Python 寻找 `site-packages` 的位置
2. **无 pip 内建** — 嵌入式 Python 不包含 pip
3. **无 venv** — 因为 Py_SetPythonHome 会覆盖标准的 site 寻找逻辑
...
Sandbox 已内置 PySide2 (用于 `_CryQt`)。安装独立的 PySide2 可能造成冲突。
